...
class ProductDetailPresenter @Inject constructor():BasePresenter<ProductDetailView>() {
    ...
    internal fun getProductDetails(id:String,user_id : String) {
        mvpView?.showProgress()
       mSubscription= mRestService.getOfferDetails(id,user_id)
               ...
               .observeOn(AndroidSchedulers.mainThread()).subscribe({ CheckNumberReponse ->
                   if (isViewAttached) {
                       ...
                       if (CheckNumberReponse.success == ApiConstants.STATUS_SUCCESS_1) {
                           mvpView?.onProductDetailFound(CheckNumberReponse)
                       } else if (CheckNumberReponse.success == ApiConstants.STATUS_SUCCESS_0) {
                           mvpView?.onProductDetailNotFound(CheckNumberReponse.error_message)
                       }
                   }}, { throwable ->
                    if (isViewAttached) {
                        mvpView?.hideProgress()
