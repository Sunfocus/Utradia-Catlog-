...
class ShopsPresenter @Inject constructor():BasePresenter<ShopsView>() {
    ...
    internal fun getShops() {
        mvpView?.showProgress()

       mSubscription= mRestService.getShops()
                ...
                .subscribe({ response ->
                    ...
                    if (isViewAttached) {
                        ...
                        if (response.success == ApiConstants.STATUS_SUCCESS_1) {
                            ...
                            mvpView?.onShopsFound(response)
                        } else if (response.success== ApiConstants.STATUS_SUCCESS_0) {
                                mvpView?.onShopsNotFound(response.error_message)
                        }
                    }
