...
class RatingReviewPresenter @Inject constructor():BasePresenter<RatingReviewView>()
{
    @Inject
    ...
    internal fun getReviewList(product_id:String) {
        mvpView?.showProgress()

        mSubscription= mRestService.getReviewList(product_id)
                ...
                .subscribe({ response ->
                    ...
                    if (isViewAttached) {
                        ...
                        if (response.success == ApiConstants.STATUS_SUCCESS_1) {
                            ...
                            mvpView?.onReviewFound(response)
