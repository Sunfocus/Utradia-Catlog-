...
class OrdersDetailPresenter @Inject constructor():BasePresenter<OrdersDetailView>()
{
    @Inject
    ...
    internal fun getOrderList(user_id:String,order_id:String) {
        mvpView?.showProgress()

        mSubscription= mRestService.getOrderDetail(user_id,order_id)
                ...
                .subscribe({ response ->
                    ...
                    if (isViewAttached) {
                        ...
                        if (response.success == ApiConstants.STATUS_SUCCESS_1) {
                            ...
                            mvpView?.orderDetail(response)
                        } else if (response.success== ApiConstants.STATUS_SUCCESS_0) {
                            mvpView?.onErrorOccur(response.error_message)
                        }
                    }
