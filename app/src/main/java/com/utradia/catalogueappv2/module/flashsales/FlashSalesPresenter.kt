...
class FlashSalesPresenter @Inject constructor():BasePresenter<FlashSalesView>() {
    ...
    internal fun getFlashSales() {
        mvpView?.showProgress()
        mSubscription=  mRestService.getFlashSales()
                ...
                .subscribe({ response ->
                    ...
                    if (isViewAttached) {
                        ...
                        if (response.success == ApiConstants.STATUS_SUCCESS_1) {
                            ...
                            mvpView?.onFlashSalesFound(response)
                        } else if (response.success== ApiConstants.STATUS_SUCCESS_0) {
                            mvpView?.onFlashNotFound(response.error_message)
                        }
                    }
